% losorelli_08_classifyFreqDomain_mag.m
% ---------------------------------------------
...
% domain (reported in Figure 3B). Loaded data are 500-sweep averaged
...
X_mag = abs(X_FFT_1000);

% Transpose back to trial-by-feature
X_use = transpose(X_mag);
...
% Save output
cd(outDir)
fnOutIntact = ['losorelli_08_classifyFreqDomain_mag_intact_' datestr(now, 'yyyymmdd_HHMM') '.mat']
save(fnOutIntact, 'C')
...
% Save output
cd(outDir)
fnOutPerm = ['losorelli_08_classifyFreqDomain_mag_permuted_' datestr(now, 'yyyymmdd_HHMM') '.mat']
save(fnOutPerm, 'C');
