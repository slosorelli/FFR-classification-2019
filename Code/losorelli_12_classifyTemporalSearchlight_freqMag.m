...
% (Supplementary Figure S3). The first part of the script performs the 
...
for i = 1:nWins
    ...
    % Get magnitude
    thisX_mag = abs(thisX_1000);
    
    % Transpose back to trial-by-feature
    thisX_use = transpose(thisX_mag);
...
% Save output
cd(outDir)
fnOutIntact = ['losorelli_12_classifyTemporalSearchlight_freqMag_intact_' datestr(now, 'yyyymmdd_HHMM') '.mat']
...
% Save output
cd(outDir)
fnOutPerm = ['losorelli_12_classifyTemporalSearchlight_freqMag_permuted_' datestr(now, 'yyyymmdd_HHMM') '.mat']
save(fnOutPerm, 'C')
